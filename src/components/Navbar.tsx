...
import { Menu, X, Construction, Phone, Mail, MapPin, ChevronDown } from 'lucide-react';
...
const navItems = [
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-[#8A0707] via-[#C80F0F] to-[#FF2E2E] text-white text-primary-foreground py-2 overflow-hidden">
        <div className="animate-marquee whitespace-nowrap flex items-center space-x-8">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium">+91 7289938603, 9136238205</span>
          </div>
          <div className="flex items-center space-x-2">
            <Mail className="h-4 w-4" />
            <span className="text-sm font-medium">Anantkumaryadav612@gmail.com</span>
          </div>
          <div className="flex items-center space-x-2">
            <MapPin className="h-4 w-4" />
            <span className="text-sm font-medium">D-90 Sector-48 Noida U.P 201031</span>
          </div>
      ...
      {/* Main Navigation */}
      <nav className="fixed top-8 w-full bg-background/95 backdrop-blur-lg border-b border-border/50 z-50 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="p-3 from-primary to-secondary rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
                
              </div>
              <div className="flex flex-col">
                <span className="font-heading font-bold text-2xl  from-primary to-secondary bg-clip-text text-transparent">
                 <img src="./images/c.png" alt="" width="120px" height="100px" />
                </span>
                ...
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className={cn(
                      "relative px-4 py-2 text-sm font-semibold rounded-lg transition-all duration-300 after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-primary after:scale-x-0 after:transition-transform after:duration-300 hover:after:scale-x-100 hover:bg-primary/10 bg-transparent",
                      isActive('/services') 
                        ? "text-primary after:scale-x-100 bg-primary/10" 
                        : "text-foreground hover:text-primary"
                    )}>
            ...
            <div className="md:hidden py-6 border-t border-border/50 bg-background/95 backdrop-blur-lg">
              <div className="flex flex-col space-y-4">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive(item.path) ? "text-primary bg-primary/10" : "text-foreground hover:text-primary hover:bg-primary/5"
                    )}
                  >
                    {item.name}
                  </Link>
                ...
                <div className="px-4 py-3">
                  <div className="text-sm font-semibold text-primary mb-2">Services</div>
                  <div className="ml-4 space-y-2">
                    <div className="text-xs font-medium text-muted-foreground mb-1">Core Cutting Service</div>
                    ...
                    <div className="text-xs font-medium text-muted-foreground mb-1 mt-3">Rebaring Service</div>
